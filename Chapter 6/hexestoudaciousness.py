...
def hexes_to_udaciousness(n, spread, target):
    global hexamesters 			# Since this function must be called recursively, using global
    final = 0					# function to avoid setting hexamesters to 0 after every function call.
    if n >= target:				# If the growth matches or exceeds the target, sets hexamester back to 0
        final = hexamesters		# and returns final (final set to hexamesters).
        hexamesters = 0
        return final
    else:						# If n < target, increases n by spread through a recursive call. 
        hexamesters += 1
