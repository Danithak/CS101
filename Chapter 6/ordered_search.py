...
def ordered_search(index, ranks, keyword):  # Uses same input as Lucky Search.
    if keyword in index:
        bestlist = index[keyword]          # Puts webpages that have the keyword inside of a list.
        pivot = [ranks[bestlist[0]]]       # Uses first page in created list as pivot point for rankings.
        bestlists = bestlist[0]            # Puts the pivot point inside of bestlists 
        bestlist.remove(bestlist[0])       # and then removes it from bestlist.
        smaller = []                       # Variable for all ranks below pivot point.
        larger = []                        # Variable for all ranks above pivot point. 
        for entry in bestlist: 
            if ranks[entry] > pivot:        
                larger += [[entry, ranks[entry]]]      # Adds pages with higher rank to larger list.
            else:
                smaller += [[entry, ranks[entry]]]     # Adds pages with lower rank to smaller list. 
        largest = 0             
        newlist = []            # Creates new list to add each page to list in order. 
        for entry in smaller:   
            if entry[1] > largest:        # Since rankings in the list are not yet ordered, this either
                newlist.append(entry[0])  # appends it to the end (if it is larger) or adds it in front if smaller. 
            else:
                newlist.insert(0, entry[0])
        smaller = newlist       # Updates smaller to newlist.
        largest = 0             # Resets largest. 
        newlist = []            # Resets newlist. 
        for entry in larger:
            if entry[1] > largest:          # Goes through the same process to put the larger 
                newlist.append(entry[0])    # in order. 
            else:
                newlist.insert(0, entry[0])
        larger = newlist
        smaller.append(bestlists) # Appends pivot point to small list.
        smaller += larger       # Appends larger list to small list after pivot point. 
        return smaller          
    else:
        return None
...
# Rankings of each page, examples outputs at bottom. 
...
# The professor's solution is far more elegant, using a recursive function call. While the code
# is more elegant, the runtime is not necessarily smaller, as the function will have to continue
# calling itself to sort the smaller and larger lists using the pivot point.
...
def ordered_search_prof(index, ranks, keyword):
