...
def crawl_web(seed,max_depth):
    tocrawl = [seed]
    crawled = []
    next_depth = []         # By creating next_depth, this allows us to measure when we have crawled a full level of depth
    depth = 0               # Initial depth level
    while tocrawl and depth <= max_depth:   # Ensures we do not crawl past the depth level specified
        page = tocrawl.pop() 
        if page not in crawled:
            union(next_depth, get_all_links(get_page(page)))    # Pulls all of the next level links and places them in the next depth level
            crawled.append(page)        
        if not tocrawl:
            tocrawl, next_depth = next_depth, []       # Allows crawling to continue, tocrawl becomes the next depth level in which to crawl.
            depth += 1      # Increases depth counter
    return crawled
