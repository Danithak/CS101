...
# Created a function to reliably check whether two pages have reciprocal links at a depth of k. 

def is_reciprocal_link(graph, source, destination, k):
    if k == 0:
        if destination == source: # Simple check to see if a page links to itself. 
            return True
        return False
    if source in graph[destination]: # One level deep: checks if source is on destination page 
        return True                  # when moving to it from the source. 
    for node in graph[destination]:  # Checks if the source link is inside of any of the destination
        if is_reciprocal_link(graph, source, node, k - 1): # link's pages at a depth of k -1 
            return True                   # (as the first level was checked in line 36.)
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_reciprocal_link(graph, node, page, k): # Added line as an additional check
                        newrank = newrank + d * (ranks[node]/len(graph[node])) #  before increasing rank. 
