...
# This procedure adds another value into the index for each url: the amount of times
# the link has been clicked. It searches through the index for the keyword (line 47 - 48)
# and after finding it, loops through the entry to find the input url (line 51),
# and then increments the clicked on counter (line 53).

def record_user_click(index,keyword,url):
	for entry in index:
		if keyword in entry:
			i = 0 			# i used to loop through every keyword
			while i < len(entry[1]):
				if url in entry[1][i]:
					entry[1][i][1] += 1 # Adds click
				i +=1 		# i incremented to search through entire entry
