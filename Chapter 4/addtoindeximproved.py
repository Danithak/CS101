...
def add_to_index(index, keyword, url):
    for entry in index:
        if entry[0] == keyword:         # Checks to see if keyword is already present.
            if url not in entry[1]:     # Does a check for the url, appends it to 
                entry[1].append(url)    # the entry if so, adds the keyword with the 
            return                      # url attached to the index if not. 
