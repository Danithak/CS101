...
def is_offered(courses, course, hexamester): 		# Function uses the previous one listed above
    x = courses_offered(courses, hexamester)		# to check if a course is offered on a given 
    if course in x:									# hexamester and returns True if so, False if not. 
        return True
    else:
        return False
