...
def involved(courses, person):		# This function essentially creates another dictionary from a dictionary.
  associate = {}					# Start with empty list.
  for hexamester in courses:			# Loop through courses in hexamester. 
    for course in courses[hexamester]:	# Loop through each course.
      if courses[hexamester][course]['teacher'] == person:	# Each teacher is listed under the 'teacher' key,
        if hexamester in associate:					# so this search is tailored towards that key. 
          associate[hexamester].append(course)		# Append value to key if found.
        else:										# 
          associate[hexamester] = [course]			# Create key and value in dictionary if not found.
      if 'assistant' in courses[hexamester][course]:	# 
        if courses[hexamester][course]['assistant'] == person: 	# Each associate is listed under the "associate" key,
          if hexamester in associate:				# so this search is tailored towards that key.
            associate[hexamester].append(course) 	# Append if key is present.
          else:
            associate[hexamester] = [course]		# Add key and value if not. 
  return associate			# Returns an empty dictionary if no teacher or associate in question is found. 
